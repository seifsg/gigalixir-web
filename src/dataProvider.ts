...
const dataProvider = <T extends DataProviderType>(type: T, resource: string, params: DataProviderParams): Promise<{}> => {
    ...
    if (isUpdate(params, type)) {
        if (resource === 'apps') {
            return apps.scale(params.id, params.data.size, params.data.replicas)
                .then((response) => {
